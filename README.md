...
**Sürüm 1.2.1** — WebGL2 tabanlı 3D sosyal çıkarım ve ortak görev oyunu. Oyuncular zaman treninde farklı evren duraklarını geçer; her durakta dünya kuralı değişir, beş ortak görev açılır ve gizli Parazitler zaman çizgisini bozmaya çalışır.
...
4. Başlatıcı önce dosyaları doğrular. 8080 kullanımda ise otomatik olarak 8081, 8082 gibi boş bir porta geçer.
5. Sağlık kontrolü başarılı olunca doğru localhost adresi tarayıcıda açılır. Menüde **Multiplayer Sunucusu** alanını boş bırak; eski bir adres kayıtlıysa **Otomatik** düğmesine bas.
6. Sunucu kapanırsa pencere açık kalır ve hata `chronorail-server.log` dosyasına yazılır.
...
## v1.2.1 bağlantı ve dağıtım düzeltmesi

- Windows başlatıcısı artık hata aldığında kapanmaz; nedeni ekranda bırakır ve `chronorail-server.log` üretir.
- 8080 portu başka program tarafından kullanılıyorsa otomatik olarak sonraki boş port seçilir.
- Sunucu başlamadan önce `public/index.html`, stil ve istemci JavaScript dosyaları doğrulanır.
- Render build aşaması eksik dosya varsa yayını durdurup açık hata verir.
- Statik oyun dizini normal repo düzenine ek olarak bir seviye iç içe yüklenmiş paketlerde de aranır.
- GitHub yüklemesi için arşiv içeriği doğrudan repo köküne çıkarılabilen düz paket sağlanır.

### Render adresinde yalnızca “Not found” görünüyorsa

Bu, Node sunucusunun çalıştığını fakat `public/index.html` dosyasının deploy edilen çalışma dizininde bulunamadığını gösterir. GitHub reposunda aşağıdaki dört öğe aynı seviyede görünmelidir:
...
Dosyalar `time-paradox-train-v1.2.1/` adlı bir alt klasördeyse iki çözüm vardır: dosyaları repo köküne taşı veya Render servisinin **Root Directory** değerini tam olarak bu alt klasör adına ayarla. Sonra **Manual Deploy → Clear build cache & deploy** çalıştır.
...
1. `start-windows.bat` penceresinin açık olduğundan ve `ChronoRail 1.2.1 hazır: http://localhost:PORT` yazdığından emin ol. PORT, 8080 kullanımda ise otomatik değişebilir.
