...
function findPublicDir() {
  ...
  // GitHub'a bir üst klasörle yüklenen paketlerde statik dizini bir seviye aşağıda da ara.
  for (const base of new Set([__dirname, process.cwd()])) {
    try {
      for (const entry of fs.readdirSync(base, { withFileTypes: true })) {
        if (!entry.isDirectory() || entry.name === 'node_modules' || entry.name.startsWith('.')) continue;
        candidates.push(path.join(base, entry.name, 'public'));
      }
    } catch {}
  }
  ...
  return path.resolve(candidates[0] || path.join(__dirname, 'public'));
}
...
function installationProblems() {
  const required = ['index.html', 'styles.css', path.join('src', 'engine.js'), path.join('src', 'game.js')];
  return required.filter((relativePath) => !fs.existsSync(path.join(PUBLIC_DIR, relativePath)));
}

function deploymentErrorPage(missing) {
  const items = missing.map((item) => `<li><code>public/${item.replaceAll('\\', '/')}</code></li>`).join('');
  return `<!doctype html><html lang="tr"><meta charset="utf-8"><title>ChronoRail kurulum hatası</title>
  <style>body{font:16px system-ui;background:#07111c;color:#e8f7ff;padding:40px;max-width:760px;margin:auto}code{color:#54e5ff}h1{color:#ffbd3e}</style>
  <h1>ChronoRail statik oyun dosyaları bulunamadı</h1>
  <p>Sunucu çalışıyor ancak GitHub/Render dağıtımında aşağıdaki dosyalar eksik veya yanlış klasörde:</p><ul>${items}</ul>
  <p><code>package.json</code>, <code>server.mjs</code>, <code>render.yaml</code> ve <code>public/</code> aynı repo kökünde olmalıdır.</p>
  <p>Render kullanıyorsan servis türü <strong>Web Service</strong>, başlangıç komutu <code>npm start</code> olmalıdır.</p></html>`;
}
...
function serveStatic(req, res) {
  ...
  const missing = installationProblems();
  if (parsed.pathname === '/healthz' || parsed.pathname === '/api/status') {
    const body = JSON.stringify({
      ...
      staticReady: missing.length === 0,
      missing
    });
    res.writeHead(missing.length ? 503 : 200, {
  ...
  if (missing.length) {
    const body = deploymentErrorPage(missing);
  ...
  fs.stat(filePath, (err, stat) => {
    if (!err && stat.isFile()) return sendFile(req, res, filePath);
    const acceptsHtml = String(req.headers.accept || '').includes('text/html');
    if (acceptsHtml && path.extname(requestPath) === '') return sendFile(req, res, INDEX_FILE);
    res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' }).end('Not found');
  });
}
...
export function createServer() {
  const missing = installationProblems();
  if (missing.length) {
    const detail = missing.map((item) => `public/${item.replaceAll('\\', '/')}`).join(', ');
    throw new Error(`Eksik oyun dosyaları: ${detail}. Repo kökünü ve public klasörünü kontrol et.`);
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  try {
    ...
    console.error('[KURULUM HATASI]', error.message);
    console.error(`Çalışma dizini: ${process.cwd()}`);
    console.error(`Sunucu dizini: ${__dirname}`);
    console.error(`Aranan public dizini: ${PUBLIC_DIR}`);
    process.exit(1);
  }
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`Statik dosyalar: ${PUBLIC_DIR}`);
